...
export function createTestWebSocketRoute(log: Logger, withAbstract: boolean) {
	if (withAbstract) {
		class WSR extends C.WebSocketRoute {
			...
			override onOpen?: C.WebSocketRouteDefinition["onOpen"] | undefined = (ws) => {
			...
			override onClose?: C.WebSocketRouteDefinition["onClose"] | undefined = (
				_ws,
				code,
				reason,
			) => {
			...
			override onMessage: C.WebSocketRouteDefinition["onMessage"] = (ws, message) => {
