...
const VALI = (process.env.VALI as never) ?? "arktype";
...
export async function startServer(PORT: number = 9876) {
	...
	// DELETE /users/:id
	new C.Route({ method: "DELETE", endpoint: "/users/:id" }, (c) => ({ deleted: c.params.id }), {
		params: UserParams,
	});

	// POST /users/:id/posts — create post for user
	new C.Route(
		{ method: "POST", endpoint: "/users/:id/posts" },
		(c) => ({
			id: 1,
	...
	new C.Route(
		{ method: "PUT", endpoint: "/orgs/:orgId/members/:memberId" },
	...
	new C.Route(
		{ method: "DELETE", endpoint: "/orgs/:orgId/members/:memberId" },
