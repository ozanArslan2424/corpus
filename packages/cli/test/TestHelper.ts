...
import { makeLogger, makeNoopLogger, type Logger } from "@/utils/logger";

export class TestHelper {
	constructor(silent: boolean) {
		this.log = silent ? makeNoopLogger() : makeLogger();
	}
	...
	logResults(title: string) {
		...
		if (this.failed > 0) {
			logger.success(`${this.passed} pass`);
			logger.error(`${this.failed} fail`);
			logger.bold("Failures:");
			for (const f of this.failures) logger.step(f);
		} else {
			logger.success(`${this.passed} pass 🔥 (${title})`);
		}
