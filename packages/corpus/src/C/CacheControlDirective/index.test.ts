...
describe("CacheControlDirective", () => {
	describe("createHeaderString", () => {
		it("SERIALIZES PUBLIC", () => {
			expect(C.CacheControlDirective.createHeaderString({ public: true })).toBe("public");
		});

		it("SERIALIZES MAX AGE", () => {
			expect(C.CacheControlDirective.createHeaderString({ maxAge: 3600 })).toBe("max-age=3600");
		});

		it("SERIALIZES MAX AGE ZERO", () => {
			expect(C.CacheControlDirective.createHeaderString({ maxAge: 0 })).toBe("max-age=0");
		});

		it("SERIALIZES IMMUTABLE", () => {
			expect(C.CacheControlDirective.createHeaderString({ immutable: true })).toBe("immutable");
		});

		it("SERIALIZES COMBINATION IN ORDER", () => {
			expect(
				C.CacheControlDirective.createHeaderString({
					immutable: true,
					maxAge: 31536000,
					public: true,
				}),
		...
		it("EMPTY DIRECTIVE SERIALIZES TO EMPTY STRING", () => {
			expect(C.CacheControlDirective.createHeaderString({})).toBe("");
		});
	});
	...
	describe("applyHeader", () => {
		it("SETS THE CACHE CONTROL HEADER", () => {
			...
			C.CacheControlDirective.applyHeader(headers, { public: true, maxAge: 3600 });
		...
		it("RETURNS THE SAME HEADERS INSTANCE", () => {
			const headers = new Headers();
			const returned = C.CacheControlDirective.applyHeader(headers, { noCache: true });
		...
		it("OVERWRITES AN EXISTING HEADER", () => {
			...
			C.CacheControlDirective.applyHeader(headers, { public: true, maxAge: 60 });
	...
	describe("constructor", () => {
		it("ASSIGNS OPTS TO THE INSTANCE", () => {
			const directive = new C.CacheControlDirective({ public: true, maxAge: 3600 });
		...
		it("INSTANCE WORKS WITH createHeaderString", () => {
			...
			expect(C.CacheControlDirective.createHeaderString(directive)).toBe("public, max-age=3600");
		});
	});
});
