...
async function testContentType(file: string, contentType: string, contentSnippet: string) {
	const r = new C.StaticRoute(`/${file}/${contentType}`, f(file));
...
describe("StaticRoute", () => {
	it("constructor", () => {
		...
		const route = new C.StaticRoute("POST /sr1", f("sample.html"), undefined, model);
	...
	it("custom static route is also available through C.Route and X.File.bunFile", async () => {
		const content = new X.File(f("sample.txt")).bunFile;
		const r = new C.Route("/natively-handled", () => new C.Res(content));
		const res = await s.handle(r.request({}));
		expect(res.status).toBe(200);
	...
	describe("custom handler", () => {
		it("receives content and can modify it", async () => {
			const r = new C.StaticRoute("/sr-custom", f("sample.txt"), (c, content) => {
				// trim for trailing \n
		...
		it("can set response status", async () => {
			const r = new C.StaticRoute("/sr-custom-status", f("sample.txt"), (c, content) => {
				c.res.status = 202;
				return content;
			});
			const res = await s.handle(r.request({}));
