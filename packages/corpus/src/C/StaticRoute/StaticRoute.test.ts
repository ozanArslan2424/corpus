...
async function testContentType(file: string, contentType: string, contentSnippet: string) {
	const r = new StaticRoute(`/${file}/${contentType}`, f(file));
...
describe("StaticRoute", () => {
	it("constructor", () => {
		...
		const route = new StaticRoute("POST /sr1", f("sample.html"), undefined, model);
	...
	it("returns 404 when file does not exist", async () => {
		new StaticRoute("/sr-missing", f("does-not-exist.html"));
		const res = await s.handle(req("/sr-missing"));
		expect(res.status).toBe(404);
	});

	it("custom static route is also available through Route and XFile.bunFile", async () => {
		const content = new XFile(f("sample.txt")).bunFile;
		const r = new Route("/natively-handled", () => new Res(content));
		const res = await s.handle(r.request({}));
		expect(res.status).toBe(200);
	...
	describe("mime types and content", async () => {
		...
		it("sets content length header", async () => {
			new StaticRoute("/sr-content-length", f("sample.txt"));
		...
		// 	new StaticRoute("/sr-ts", f("sample.ts"));
	...
	describe("custom handler", () => {
		it("receives content and can modify it", async () => {
			const r = new StaticRoute("/sr-custom", f("sample.txt"), (c, content) => {
				// trim for trailing \n
		...
		it("can set response status", async () => {
			const r = new StaticRoute("/sr-custom-status", f("sample.txt"), (c, content) => {
				c.res.status = 202;
				return content;
			});
			const res = await s.handle(r.request({}));
