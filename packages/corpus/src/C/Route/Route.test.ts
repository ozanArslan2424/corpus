...
describe("Route", () => {
	...
	it("string definition defaults to get", () => {
		...
		expect(route.id).toBe(`${Method.GET} ${path}`);
	});

	it("object definition with method", () => {
		const path = "/r2";
		const route = new Route({ method: Method.POST, path }, handler);

		expect(route.method).toBe(Method.POST);
		expect(route.endpoint).toBe(path);
		expect(route.id).toBe(`${Method.POST} ${path}`);
	});

	it("registers to router", async () => {
		const path = "/r5";
		new Route(path, () => "registered");
	...
	it("registers correct method from address (object)", async () => {
		new Route({ method: Method.POST, path: "/r6" }, () => "posted");
	...
	it("with model", () => {
		...
		const route = new Route(path, handler, model);
	...
	it.each(Object.values(Method))("method %s resolves correctly", (method) => {
	...
	it("using extended abstract method", async () => {
		...
		class MyRoute extends RouteAbstract {
			constructor() {
				super();
				this.register();
			}
