...
function walkMarkdown(dir: string): Array<string> {
	const out: Array<string> = [];
	for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
		const abs = path.join(dir, entry.name);
		if (entry.isDirectory()) {
		...
		if (entry.name.endsWith(".md")) {
			out.push(path.relative(OUT_DIR, abs).split(path.sep).join("/"));
		}
	}
	return out;
}
...
function check() {
	const expected = new Set(
		walkSources(SRC_DIR, IGNORE)
			.map((abs) => extractModule(abs, path.relative(SRC_DIR, abs)))
			.filter((module) => module.entries.length > 0 || module.description.length > 0)
			.map((module) => fileOf(module.rel)),
	);

	const actual = new Set(walkMarkdown(OUT_DIR));
...
if (flag === "check") {
	check();
} else if (flag === "gen" || flag === "generate") {
	generate();
} else {
	console.log("flag must be 'check' / 'gen' / 'generate'");
	process.exit(1);
}
