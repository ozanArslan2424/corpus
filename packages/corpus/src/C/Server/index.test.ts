...
describe("Server", () => {
	...
	describe("handle", () => {
		...
		it("returns res instance directly when handler returns res", async () => {
			const s = createTestServer();
			new C.Route("/srv-res-direct", () => {
				return new C.Res({ direct: true }, { status: 201 });
			});
		...
		it("wraps plain handler result in res with 200", async () => {
			const s = createTestServer();
			new C.Route("/srv-plain", () => "plain text");
		...
		it("passes parsed params to context", async () => {
			const s = createTestServer();
			new C.Route<never, never, { id: number }>("/srv-user/:id", (ctx) => ({ id: ctx.params.id }));
		...
		it("passes search params to context", async () => {
			const s = createTestServer();
			new C.Route<never, { q: string }>("/srv-search", (ctx) => ({ q: ctx.search.q }));
	...
	describe("method routing", () => {
		it("get and post on same path are distinct", async () => {
			const s = createTestServer();
			new C.Route("/srv-methods", () => "got");
			new C.Route({ method: "POST", path: "/srv-methods" }, () => "posted");
	...
	describe("preflight", () => {
		...
		it("uses cors preflight handler when cors is set", async () => {
			const s = createTestServer();
			new C.Cors({ allowedOrigins: ["https://example.com"] });
			const res = await s.handle(
	...
	describe("handleError", () => {
		it("custom handler is called on error", async () => {
			...
			s.handleError = () => {
				return new C.Res({ error: true, message: "custom error" }, { status: 500 });
			};
		...
		it("http error is handled by default handler", async () => {
			...
			new C.Route("/srv-httperror", () => {
				throw new C.Exception("bad input", C.Status.BAD_REQUEST);
			});
		...
		it("custom handler receives error and context", async () => {
			...
			s.handleError = (err, ctx) => {
				...
				return new C.Res({ error: true }, { status: 500 });
			};
			new C.Route("/srv-error-ctx", () => {
		...
		it("error thrown in middleware is handled", async () => {
			...
			new C.Route("/srv-mw-error", () => "never");
	...
	describe("handleNotFound", () => {
		it("custom handler is called", async () => {
			...
			s.handleNotFound = () => {
				return new C.Res({ error: true, message: "custom not found" }, { status: 404 });
			};
