...
describe("CacheControlDirective", () => {
	describe("createHeaderString", () => {
		it("SERIALIZES PUBLIC", () => {
			expect(TC.CacheControlDirective.createHeaderString({ public: true })).toBe("public");
		});

		it("SERIALIZES MAX AGE", () => {
			expect(TC.CacheControlDirective.createHeaderString({ maxAge: 3600 })).toBe("max-age=3600");
		});

		it("SERIALIZES MAX AGE ZERO", () => {
			expect(TC.CacheControlDirective.createHeaderString({ maxAge: 0 })).toBe("max-age=0");
		});

		it("SERIALIZES IMMUTABLE", () => {
			expect(TC.CacheControlDirective.createHeaderString({ immutable: true })).toBe("immutable");
		});

		it("SERIALIZES COMBINATION IN ORDER", () => {
			expect(
				TC.CacheControlDirective.createHeaderString({
					immutable: true,
					maxAge: 31536000,
					public: true,
				}),
		...
		it("EMPTY DIRECTIVE SERIALIZES TO EMPTY STRING", () => {
			expect(TC.CacheControlDirective.createHeaderString({})).toBe("");
		});
	});
	...
	describe("applyHeader", () => {
		it("SETS THE CACHE CONTROL HEADER", () => {
			...
			TC.CacheControlDirective.applyHeader(headers, { public: true, maxAge: 3600 });
		...
		it("RETURNS THE SAME HEADERS INSTANCE", () => {
			const headers = new Headers();
			const returned = TC.CacheControlDirective.applyHeader(headers, { noCache: true });
		...
		it("OVERWRITES AN EXISTING HEADER", () => {
			...
			TC.CacheControlDirective.applyHeader(headers, { public: true, maxAge: 60 });
	...
	describe("constructor", () => {
		it("ASSIGNS OPTS TO THE INSTANCE", () => {
			const directive = new TC.CacheControlDirective({ public: true, maxAge: 3600 });
		...
		it("INSTANCE WORKS WITH createHeaderString", () => {
			...
			expect(TC.CacheControlDirective.createHeaderString(directive)).toBe("public, max-age=3600");
		});
	});
});
