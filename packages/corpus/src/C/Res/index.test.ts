...
describe("Res", () => {
	const ctHeader = C.HeaderKey.ContentType;
	const locHeader = C.HeaderKey.Location;
	const locUrl = "/hello";
	...
	describe("static methods", () => {
		it("redirect", async () => {
			const res = C.Res.redirect(locUrl);
		...
		it("permanentRedirect", async () => {
			const res = C.Res.permanentRedirect(locUrl);
			const response = res.response;
			expect(res.headers.get(locHeader)).toBe(locUrl);
			expect(response.headers.get(locHeader)).toBe(locUrl);
			expect(res.body).toBeUndefined();
		...
		it("temporaryRedirect", async () => {
			const res = C.Res.temporaryRedirect(locUrl);
			const response = res.response;
			expect(res.headers.get(locHeader)).toBe(locUrl);
			expect(response.headers.get(locHeader)).toBe(locUrl);
			expect(res.body).toBeUndefined();
		...
		it("seeOther", async () => {
			const res = C.Res.seeOther(locUrl);
			const response = res.response;
			expect(res.headers.get(locHeader)).toBe(locUrl);
			expect(response.headers.get(locHeader)).toBe(locUrl);
			expect(res.body).toBeUndefined();
		...
		it("sse", async () => {
			const res = C.Res.sse((send) => {
				send({ event: "ping", data: { time: 1 } });
			});
			const response = res.response;
			expect(res.body).toBeInstanceOf(ReadableStream);
			expect(res.headers.get(C.HeaderKey.CacheControl)).toBe("no-cache");
			expect(res.headers.get(C.HeaderKey.Connection)).toBe("keep-alive");
			expectData({
				res,
				response,
				expectedOK: true,
		...
		it("sse async - stream emits correct chunks", async () => {
			const res = C.Res.sse(async (send) => {
				send({ event: "ping", data: { time: 1 } });
				send({ id: "2", event: "pong", data: { time: 2 } });
			});
		...
		it("ndjson async  - returns stream with correct headers", () => {
			const res = C.Res.ndjson(async (send) => {
				send({ id: 1 });
			});
			const response = res.response;
		...
		it("streamfile - returns stream with correct headers for txt", async () => {
			const res = await C.Res.streamFile("test/fixtures/sample.txt");

			expect(res.status).toBe(C.Status.OK);
			expect(res.body).toBeInstanceOf(ReadableStream);
			expect(res.headers.get(ctHeader)).toBe("text/plain");
			expect(res.headers.get(C.HeaderKey.ContentDisposition)).toBe(
		...
		it("streamfile - infers correct mime type", async () => {
			...
			for (const [path, expectedMime] of cases) {
				const res = await C.Res.streamFile(path);
				expect(res.headers.get(ctHeader)).toBe(expectedMime);
			}
		});

		it("streamfile - inline disposition", async () => {
			const res = await C.Res.streamFile("test/fixtures/sample.txt", "inline");
			expect(res.headers.get(C.HeaderKey.ContentDisposition)).toBe(
		...
		it("streamfile - throws not found for missing file", () => {
			expect(C.Res.streamFile("test/fixtures/does-not-exist.txt")).rejects.toThrow();
		});
	...
	describe("body", () => {
		...
		it("bigint body", async () => {
			const res = new C.Res(9007199254740993n);
			const response = res.response;
