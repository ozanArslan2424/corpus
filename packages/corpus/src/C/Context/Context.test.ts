...
const plainHeaders = {
	...
	[HeaderKey.Cookie]: toCookieHeader(plainCookies),
};
...
const server = new Server({ hostname: TEST_HOST, port: TEST_PORT });

// stable context ref to avoid actual request handling
let context = new Context(request, server.app);
...
describe("Context", () => {
	...
	describe("members - after handle - no model", async () => {
		const route = new Route<unknown, unknown, unknown, unknown>(
	...
	describe("lazy members - initial", () => {
		...
		it("get res", () => {
			expect(context.res).toBeInstanceOf(Res);
		});

		it("set res", () => {
			expect(context.res.status).toBe(Status.OK);
			context.res = new Res("", { status: Status.INTERNAL_SERVER_ERROR });
			expect(context.res.body).toBe("");
			expect(context.res.status).toBe(Status.INTERNAL_SERVER_ERROR);
		});
