...
const server = new C.App({ port: PORT });
...
async function suite(name: string, fn: () => Promise<void>) {
	...
	try {
		...
		const msg = `Suite "${name}" threw unexpectedly: ${T.stringify(err)}`;
		T.log.error(msg);
		T.failures.push(msg);
		T.failed++;
	}
...
new C.Route(
	{ method: C.Method.POST, endpoint: "/users" },
	(c) => {
		const id = idCounter++;
		const user = {
			id,
...
new C.Route(
	{ method: C.Method.PUT, endpoint: "/users/:id" },
	(ctx) => {
		const user = db.get(ctx.params.id);
		if (!user) {
			throw new C.Exception("not found", 404);
...
new C.Route(
	{ method: C.Method.PATCH, endpoint: "/users/:id" },
	(ctx) => {
		const user = db.get(ctx.params.id);
		if (!user) {
			throw new C.Exception("not found", 404);
...
new C.Route(
	{ method: C.Method.DELETE, endpoint: "/users/:id" },
	(ctx) => {
...
new C.Route({ method: C.Method.POST, endpoint: "/echo" }, (c) => ({
	echoed: c.body,
	headers: {
		contentType: c.req.headers.get("content-type"),
		custom: c.req.headers.get("x-custom-header"),
	},
}));
