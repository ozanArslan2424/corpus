...
class TestParsingController extends C.Controller {
	constructor() {
		super("/controller");
	}
...
const postRoute = (
	...
	return s.handle(
		new Request(url, {
			...
			headers: { [C.HeaderKey.ContentType]: "application/json" },
		}),
	);
};
...
describe("Parser unit", () => {
	...
	describe("failure", () => {
		it("ark object", () => {
			expect(parse(BAD, TestModel.arkObject)).rejects.toThrow(C.Exception);
		});
		it("zod object", () => {
			expect(parse(BAD, TestModel.zodObject)).rejects.toThrow(C.Exception);
		});
		it("ark route", () => {
			expect(parse(BAD, TestModel.arkRoute.params)).rejects.toThrow(C.Exception);
			expect(parse(BAD, TestModel.arkRoute.search)).rejects.toThrow(C.Exception);
			expect(parse(BAD, TestModel.arkRoute.body)).rejects.toThrow(C.Exception);
		});
		it("zod route", () => {
			expect(parse(BAD, TestModel.zodRoute.params)).rejects.toThrow(C.Exception);
			expect(parse(BAD, TestModel.zodRoute.search)).rejects.toThrow(C.Exception);
			expect(parse(BAD, TestModel.zodRoute.body)).rejects.toThrow(C.Exception);
		});
		it("ark route (referenced schemas)", () => {
			expect(parse(BAD, TestModel.arkRouteReferenced.params)).rejects.toThrow(C.Exception);
			expect(parse(BAD, TestModel.arkRouteReferenced.search)).rejects.toThrow(C.Exception);
			expect(parse(BAD, TestModel.arkRouteReferenced.body)).rejects.toThrow(C.Exception);
		});
		it("zod route (referenced schemas)", () => {
			expect(parse(BAD, TestModel.zodRouteReferenced.params)).rejects.toThrow(C.Exception);
			expect(parse(BAD, TestModel.zodRouteReferenced.search)).rejects.toThrow(C.Exception);
			expect(parse(BAD, TestModel.zodRouteReferenced.body)).rejects.toThrow(C.Exception);
		});
	});
	...
	describe("parser.parseSync", () => {
		...
		it("throws Exception for sync validator on bad input", () => {
			expect(() => parser.parseSync("test", BAD, syncSchema)).toThrow(C.Exception);
		});
