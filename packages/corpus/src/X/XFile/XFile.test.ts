...
describe("XFile", () => {
	...
	describe("getters", () => {
		it("extension - from path", () => {
			expect(new XFile("assets/document.txt").extension).toBe("txt");
		});

		it("extension - fallback when no dot", () => {
			expect(new XFile("assets/data", "json").extension).toBe("json");
		});

		it("extension - default to txt when no dot and no fallback", () => {
			expect(new XFile("assets/data").extension).toBe("txt");
		});

		it("name - strips extension", () => {
			expect(new XFile("assets/document.txt").name).toBe("document");
		});

		it("name - handles path with no directory", () => {
			expect(new XFile("readme.md").name).toBe("readme");
		});

		it("fullname - includes extension", () => {
			expect(new XFile("assets/document.txt").fullname).toBe("document.txt");
		});

		it("dir - returns parent directory", () => {
			expect(new XFile("assets/sub/document.txt").dir).toBe("assets/sub");
		});

		it("dir - root level file", () => {
			expect(new XFile("document.txt").dir).toBe(".");
		});

		it("parent_dirs - ordered immediate to root", () => {
			expect(new XFile("a/b/c/file.txt").parentDirs).toEqual(["c", "b", "a"]);
		});
		...
		it("parent_dirs - filters empty segments from leading slash", () => {
			expect(new XFile("/a/b/file.txt").parentDirs).toEqual(["b", "a"]);
		});
	});

	describe("mimeType", () => {
		it("known extensions", () => {
			expect(new XFile("a.html").mimeType).toInclude("text/html");
			expect(new XFile("a.css").mimeType).toInclude("text/css");
			expect(new XFile("a.js").mimeType).toInclude("text/javascript");
			expect(new XFile("a.json").mimeType).toInclude("application/json");
			expect(new XFile("a.png").mimeType).toInclude("image/png");
			expect(new XFile("a.webp").mimeType).toInclude("image/webp");
			expect(new XFile("a.svg").mimeType).toInclude("image/svg+xml");
			expect(new XFile("a.pdf").mimeType).toInclude("application/pdf");
			expect(new XFile("a.mp4").mimeType).toInclude("video/mp4");
			expect(new XFile("a.mp3").mimeType).toInclude("audio/mpeg");
			expect(new XFile("a.woff2").mimeType).toInclude("font/woff2");
			expect(new XFile("a.xyz").mimeType).toBe("chemical/x-xyz");
		});

		it("unknown extension falls back to octet-stream", () => {
			expect(new XFile("a.what").mimeType).toBe("application/octet-stream");
		});

		it("uses fallback extension when path has none", () => {
			expect(new XFile("data", "json").mimeType).toBe("application/json");
		});
	});

	describe("sibling", () => {
		it("returns file in same directory", () => {
			const file = new XFile("assets/sub/document.txt");
			expect(file.sibling("other.txt").path).toBe(path.join("assets/sub", "other.txt"));
		});

		it("inherits fallback extension", () => {
			const file = new XFile("assets/data", "json");
			// @ts-expect-error
	...
	describe("withExtension", () => {
		it("returns file with new extension", () => {
			const file = new XFile("assets/document.txt");
	...
	describe("exists", () => {
		it("returns true for existing file", async () => {
			const file = new XFile(await writeFile("present.txt", "hello"));
			expect(await file.exists()).toBeTrue();
		});

		it("returns false for missing file", async () => {
			expect(await new XFile(path.join(tmpDir, "missing.txt")).exists()).toBeFalse();
		});
	});

	describe("text", () => {
		it("reads utf8 by default", async () => {
			const file = new XFile(await writeFile("text.txt", "hello world"));
			expect(await file.text()).toBe("hello world");
		});

		it("reads non-ascii content", async () => {
			const file = new XFile(await writeFile("unicode.txt", "merhaba 🌍"));
			expect(await file.text()).toBe("merhaba 🌍");
		});

		it("reads empty file", async () => {
			const file = new XFile(await writeFile("empty.txt", ""));
			expect(await file.text()).toBe("");
		});

		it("throws when file missing", () => {
			expect(new XFile(path.join(tmpDir, "nope.txt")).text()).rejects.toThrow();
		});
	});

	describe("bytes", () => {
		it("returns uint8array", async () => {
			const file = new XFile(await writeFile("bytes.txt", "hello"));
		...
		it("content matches file", async () => {
			...
			expect(await new XFile(filePath).bytes()).toEqual(data);
		});

		it("throws when file missing", () => {
			expect(new XFile(path.join(tmpDir, "nope.bin")).bytes()).rejects.toThrow();
		});
	});

	describe("stream", () => {
		it("returns a readable stream", async () => {
			const file = new XFile(await writeFile("stream.txt", "streamed contents"));
		...
		it("stream contents match file contents", async () => {
			...
			const file = new XFile(await writeFile("stream-read.txt", contents));
		...
		it("handles binary content", async () => {
			...
			const buffer = await new Response(await new XFile(filePath).stream()).arrayBuffer();
			expect(new Uint8Array(buffer)).toEqual(bytes);
		});

		it("handles empty file", async () => {
			const file = new XFile(await writeFile("empty-stream.txt", ""));
			expect(await new Response(await file.stream()).text()).toBe("");
		});

		it("throws when file missing", async () => {
			const file = new XFile(path.join(tmpDir, "missing-stream.txt"));
	...
	describe("stat", () => {
		it("returns stats object", async () => {
			const file = new XFile(await writeFile("stat.txt", "data"));
		...
		it("size matches content length", async () => {
			...
			const file = new XFile(await writeFile("stat-size.txt", content));
			const s = await file.stat();
			expect(s.size).toBe(Buffer.byteLength(content, "utf8"));
		});

		it("throws when file missing", () => {
			expect(new XFile(path.join(tmpDir, "missing-stat.txt")).stat()).rejects.toThrow();
		});
	});
	...
	describe("size", () => {
		it("returns size in bytes", async () => {
			const content = "hello";
			const file = new XFile(await writeFile("size.txt", content));
			expect(await file.size()).toBe(Buffer.byteLength(content, "utf8"));
		});

		it("returns null for missing file", async () => {
			expect(await new XFile(path.join(tmpDir, "missing-size.txt")).size()).toBeNull();
		});
	});
	...
	describe("write", () => {
		it("writes string content", async () => {
			...
			await new XFile(filePath).write("hello world");
		...
		it("writes uint8array content", async () => {
			...
			await new XFile(filePath).write(bytes);
		...
		it("writes arraybuffer content", async () => {
			...
			await new XFile(filePath).write(buffer);
			expect(new Uint8Array(await fs.readFile(filePath))).toEqual(new Uint8Array(buffer));
		});

		it("overwrites existing file", async () => {
			const file = new XFile(await writeFile("overwrite.txt", "original"));
		...
		it("creates parent directories", async () => {
			...
			await new XFile(filePath).write("nested content");
		...
		it("writes empty string", async () => {
			...
			await new XFile(filePath).write("");
	...
	describe("append", () => {
		it("appends to existing file", async () => {
			const file = new XFile(await writeFile("append.txt", "hello"));
	...
	describe("copyTo", () => {
		it("creates a copy at destination", async () => {
			const file = new XFile(await writeFile("copy-src.txt", "copy me"));
		...
		it("original still exists", async () => {
			const file = new XFile(await writeFile("copy-original.txt", "still here"));
			await file.copyTo(path.join(tmpDir, "copy-original-dest.txt"));
			expect(await file.exists()).toBeTrue();
		});

		it("returns xfile pointing to destination", async () => {
			const file = new XFile(await writeFile("copy-ret.txt", "data"));
		...
		it("creates parent directories", async () => {
			const file = new XFile(await writeFile("copy-mkdir.txt", "data"));
	...
	describe("moveTo", () => {
		it("file exists at destination", async () => {
			const file = new XFile(await writeFile("move-src.txt", "move me"));
		...
		it("creates parent directories", async () => {
			const file = new XFile(await writeFile("move-mkdir.txt", "data"));
	...
	describe("unlink", () => {
		...
		it("throws when file missing", () => {
			expect(new XFile(path.join(tmpDir, "never-existed.txt")).unlink()).rejects.toThrow();
		});

		it("exists returns false after unlink", async () => {
			const file = new XFile(await writeFile("check-after.txt", "data"));
			expect(await file.exists()).toBeTrue();
			await file.unlink();
		...
		it("write after unlink recreates file", async () => {
			const file = new XFile(await writeFile("recreate.txt", "first"));
			await file.unlink();
	...
	describe("fromBunFile", () => {
		it("accepts bunfile as input", async () => {
			...
			const file = new XFile(Bun.file(filePath));
		...
		it("exists works from bunfile", async () => {
			...
			expect(await new XFile(Bun.file(filePath)).exists()).toBeTrue();
		});

		it("missing bunfile returns false for exists", async () => {
			expect(
				await new XFile(Bun.file(path.join(tmpDir, "bunfile-missing.txt"))).exists(),
			).toBeFalse();
		});
		...
		it("fallback extension still applies from bunfile with no extension", async () => {
			...
			await fs.writeFile(filePath, "data");
			const file = new XFile(Bun.file(filePath), "json");
			expect(file.extension).toBe("json");
		});
	});
});
